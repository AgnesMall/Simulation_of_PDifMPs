...
NumericMatrix cholesky_decomp(Rcpp::NumericMatrix mat, double z, double t) {
  ...
  try {
    ...
    return Rcpp::wrap(NumericMatrix());  // Return emtpy matrix, if an error occurs
  }
...
NumericMatrix sde_SHO_cpp_t(NumericVector startv,double t_fin,double t0,double h,double sig, double z, double a){
  ...
  if(N > 0){
    for(int i=0;i<N;i++)
    {
      NumericMatrix dm  = exp_mat_Cpp(h,a,z);
      NumericMatrix cm = transpose(cholesky_decomp(cov_mat_Cpp(h,sig,a,z),z,h));
